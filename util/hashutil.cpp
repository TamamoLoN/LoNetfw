...
uint32_t HashUtil::murmur3_hash(const void *data, const uint32_t &size, const uint32_t &seed)
{
    if (!data)
        return 0;
...
uint32_t HashUtil::murmur3_hash(const char *str, const uint32_t &seed)
{
    if (!str)
        return 0;
...
uint32_t HashUtil::quick_hash(const char *str)
{
    unsigned int h = 0;
    for (; *str; str++)
    {
...
uint32_t HashUtil::quick_hash(const void *tmp, uint32_t size)
{
...
uint64_t HashUtil::murmur3_hash64(const void *str, const uint32_t &size, const uint32_t &seed,
                                  const uint32_t &seed2)
{
    return (((uint64_t)murmur3_hash(str, size, seed)) << 32 | murmur3_hash(str, size, seed2));
}
uint64_t HashUtil::murmur3_hash64(const char *str, const uint32_t &seed, const uint32_t &seed2)
{
    return (((uint64_t)murmur3_hash(str, seed)) << 32 | murmur3_hash(str, seed2));
}

std::string HashUtil::base64decode(const std::string &src)
{
    std::string result;
...
std::string HashUtil::base64encode(const std::string &data)
{
    return base64encode(data.c_str(), data.size());
}

std::string HashUtil::base64encode(const void *data, size_t len)
{
...
std::string HashUtil::md5(const std::string &data)
{
    return hexstring_from_data(md5sum(data).c_str(), MD5_DIGEST_LENGTH);
}

std::string HashUtil::sha1(const std::string &data)
{
    return hexstring_from_data(sha1sum(data).c_str(), SHA_DIGEST_LENGTH);
}

std::string HashUtil::md5sum(const void *data, size_t len)
{
    MD5_CTX ctx;
    MD5_Init(&ctx);
...
std::string HashUtil::md5sum(const std::string &data) { return md5sum(data.c_str(), data.size()); }

#if OPENSSL_VERSION_NUMBER < 0x1010000fL
std::string HashUtil::sha0sum(const void *data, size_t len)
{
    SHA_CTX ctx;
    SHA_Init(&ctx);
...
std::string HashUtil::sha0sum(const std::string &data)
{
    return sha0sum(data.c_str(), data.length());
}
#endif

std::string HashUtil::sha1sum(const void *data, size_t len)
{
    SHA_CTX ctx;
    SHA1_Init(&ctx);
...
std::string HashUtil::sha1sum(const std::string &data)
{
...
std::string HashUtil::hmac_md5(const std::string &text, const std::string &key)
{
    return hmac<MD5_CTX, &MD5_Init, &MD5_Update, &MD5_Final, MD5_CBLOCK, MD5_DIGEST_LENGTH>(text,
                                                                                            key);
}

std::string HashUtil::hmac_sha1(const std::string &text, const std::string &key)
{
    return hmac<SHA_CTX, &SHA1_Init, &SHA1_Update, &SHA1_Final, SHA_CBLOCK, SHA_DIGEST_LENGTH>(text,
                                                                                               key);
}

std::string HashUtil::hmac_sha256(const std::string &text, const std::string &key)
{
    return hmac<SHA256_CTX, &SHA256_Init, &SHA256_Update, &SHA256_Final, SHA256_CBLOCK,
                SHA256_DIGEST_LENGTH>(text, key);
}

void HashUtil::hexstring_from_data(const void *data, size_t len, char *output)
{
...
std::string HashUtil::hexstring_from_data(const void *data, size_t len)
{
    if (len == 0)
    {
        return std::string();
...
std::string HashUtil::hexstring_from_data(const std::string &data)
{
    return hexstring_from_data(data.c_str(), data.size());
}

void HashUtil::data_from_hexstring(const char *hexstring, size_t length, void *output)
{
...
std::string HashUtil::data_from_hexstring(const char *hexstring, size_t length)
{
    if (length % 2 != 0)
    {
        throw std::invalid_argument("data_from_hexstring length % 2 != 0");
...
std::string HashUtil::data_from_hexstring(const std::string &hexstring)
{
    return data_from_hexstring(hexstring.c_str(), hexstring.size());
}

void HashUtil::replace(std::string &str, char find, char replaceWith)
{
...
void HashUtil::replace(std::string &str, char find, const std::string &replaceWith)
{
    size_t index = str.find(find);
    while (index != std::string::npos)
    {
...
void HashUtil::replace(std::string &str, const std::string &find, const std::string &replaceWith)
{
    size_t index = str.find(find);
    while (index != std::string::npos)
    {
...
std::vector<std::string> HashUtil::split(const std::string &str, char delim, size_t max)
{
...
std::vector<std::string> HashUtil::split(const std::string &str, const char *delims, size_t max)
{
    std::vector<std::string> result;
    if (str.empty())
    {
...
std::string HashUtil::random_string(size_t len)
{
    if (len == 0)
    {
        return "";
